      ...
      <ul>
        {lots.length === 0 ? (
          ...
          lots.map(lot => {
            ...
            // Revenue: sum of payments for bookings in this lot
            let lotRevenue = 0;
            ...
            return (
              <li key={lot.id} style={{ marginBottom: 16, padding: 8, borderBottom: "1px solid #e0e0e0" }}>
                <strong>{lot.name}</strong> ({lot.location})<br />
                Spaces: {totalSpaces} | Occupied: {occupiedSpaces} | Occupancy Rate: {occupancyRate}%<br />
                Revenue: ${lotRevenue.toFixed(2)}<br />
                Most Used Space: {mostUsed ? `${mostUsed.space.space_number || mostUsed.space.name} (${mostUsed.count} bookings)` : "N/A"}<br />
                Least Used Space: {leastUsed ? `${leastUsed.space.space_number || leastUsed.space.name} (${leastUsed.count} bookings)` : "N/A"}
              </li>
            );
          })
        )}
